...
export default {
  ...
  methods: {
    ...
    // 一级分类的select事件回调（当一级分类的option发送变化时获取相应二级分类的数据
    async handler1() {
      // 清除二级、三级分类的旧数据
      this.list2 = []
      this.list3 = []
    ...
    // 二级分类的select事件回调（当二级分类的option发送变化时获取相应三级分类的数据）
    async handler2() {
      // 清除三级分类的旧数据
      this.list3 = []
      this.cForm.category3Id = ''
    ...
    // 三级分类的select事件回调（当三级分类的option发送变化时，触发父组件传来的自定义事件，将三个分类的id都传过去，并作以区分
    handler3() {
      // 解构出三级分类的id
      const { category3Id } = this.cForm
      this.$emit('getCategoryId', { categoryId: category3Id })
    }
  }
}
</script>
